...
const handlers = {
    ...
    "*":{
        "hi":(account,method,code,actData, events)=>{
            console.log(account,method,code,actData, events);
        },
        "*":{
            "*":(account,method,code,actData, event)=>{
    ...
    "helloworld":{
        "hi":{
            "someevent":(account,method,code,actData, event)=>{
                console.log(`got ${code}.${method}.${event.etype} with metadata`, actData,event);
            }
        }
    },
}
...
async function transactionHandler(tx){
    var actionTraces = tx.action_traces.map(a=>a[1]);
